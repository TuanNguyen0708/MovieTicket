...
const data = [
    ...
    {
        maMonAn: 'MA04',
        tenMonAn: 'Gà nướng muối tiêu xanh',
        gia: '600',
...
export default function BaiTapTongHop() {
    ...
    return (
        <div>
            <Popup openModal={openModal} setOpenModal={setOpenModal} mangDanhMuc={mangDanhMuc} item={item} />
            <div className='grid grid-cols-6'>
                <div className="h-full p-3 space-y-2 w-60 dark:bg-coolGray-900 dark:text-coolGray-100 col-span-1">
                    <div className="flex items-center p-2 space-x-4">
                        <img src="https://source.unsplash.com/100x100/?portrait" alt className="w-12 h-12 rounded-full" />
                        <div>
                            <h2 className="text-lg font-semibold">Leroy Jenkins</h2>
                            <span className="flex items-center space-x-1">
                                <a href="#" className="text-xs hover:underline dark:text-coolGray-400">View profile</a>
                            </span>
                        </div>
                    </div>
                    <div className="divide-y divide-coolGray-700">
                        <ul className="pt-2 pb-4 space-y-1 text-sm">
                            {mangDanhMuc.map((danhMuc, index) => {
                                    const active = ['flex', 'items-center', 'p-2', 'space-x-3', 'rounded-md',maDanhMuc === danhMuc.maDanhMuc ? 'bg-black text-white' : ' ']
                                return <li className="dark:bg-coolGray-800 dark:text-coolGray-50" key={index} onClick={() => {
                                    setMaDanhMuc(danhMuc.maDanhMuc);
                                }}>
                                    <a href="#" className={active.join(' ')}>
                ...
                <div className='col-span-5'>
                    <h3 className='text-center text-5xl mt-5'>Menu</h3>
                    <section class="text-gray-600 body-font">
                        <div class="container px-5 py-24 mx-auto">
                            <div class="flex flex-wrap -m-4">
                                {_.filter(data, item => item.maDanhMuc === maDanhMuc).map((item, index) => {
                                    return <div class="p-4 lg:w-1/3" key={index}>
                                        <div class="h-full bg-gray-100 bg-opacity-75 px-8 pt-16 pb-24 rounded-lg overflow-hidden text-center relative">
                                            <h2 class="tracking-widest text-xs title-font font-medium text-gray-400 mb-1">{item.tenDanhMuc}</h2>
                                            <h1 class="title-font sm:text-2xl text-xl font-medium text-gray-900 mb-3">{item.tenMonAn}</h1>
                                            <p class="leading-relaxed mb-3">{<img src={item.hinhAnh} />}</p>
                                            ...
                                            <div class="text-center mt-2 leading-none flex justify-end absolute bottom-0 left-0 w-full py-4">
                                                <span class="text-gray-400 mr-3 inline-flex items-center leading-none pr-3 py-1 border-gray-200 text-2xl">
                                                    Giá: {item.gia}
                                                </span>
