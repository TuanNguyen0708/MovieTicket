...
const MultipleRows = (props) => {
  ...
  const renderPhim = () => {
    return props.arrPhim.slice(0,20).map((item, index) => {
      return <div key={index} style={{width:'20%', padding:'0 30px'}}>
  ...
  return (
    <div className='mt-5'>
      <button type="button" className={`${styleSlick[activeClassDC]} px-8 py-3 font-semibold rounded mr-3`} style={{marginLeft:'45px'}} onClick={() => {
